# -*- coding: utf-8 -*-
"""
Created on Thu Aug  4 10:14:33 2022
...
###############################################################################
def create_psf(fwhm, eng, ndims=2, real_pixel=True):
    """ Creates a psf using the matlab functions fspecial or fspecial3 """
    fwhm = np.array([fwhm])
    
    if real_pixel == True:
        if ndims == 2:
            pixel_dim = np.array([4.0728])
        elif ndims == 3:
            pixel_dim = np.array([4.0728, 4.0728, 3.0])
        fwhm = fwhm / pixel_dim
        
    sigma = fwhm / math.sqrt(8*np.log(2))
    sigma = matlab.double(sigma.tolist())
    
    if ndims == 2:
        psf = eng.fspecial('gaussian', [], sigma)
    elif ndims == 3:
        psf = eng.fspecial3('gaussian', [], sigma)
        
    psf_p = np.array(psf)
    return psf_p

###############################################################################
def add_noise(im, norm=True):
    """ Adds poisson noise to an array """
    array = im.flatten()
    for i in range(array.shape[0]):
            if array[i] != 0 :
                noise = np.random.poisson(np.sqrt(array[i]))
                array[i] = array[i] + (random.choice((-1,1)) * noise)
    array[array < 0] = 0
    array = np.reshape(array, im.shape)
    if norm == True:
        return array / np.sum(array) * np.sum(im)
    else:
        return array
